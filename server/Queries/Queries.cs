...
public class Queries
{
    ...
    {
        await using var cmd =
            _db.CreateCommand("SELECT EXISTS(SELECT * FROM users WHERE email = $1 AND password = $2)");
    ...
    public async Task<List<TicketRecord>>
        GetTicketsAll(string email) //email för den som gjort request används för att få vilket företag
    {
    ...
    public async Task<TicketRecord>
        GetTicket(string email, int id) //email för den som gjort request används för att få vilket företag
    {
        TicketRecord ticket;
        await using var cmd =
            _db.CreateCommand(
    ...
    public async Task<bool> PostMessageTask(SendEmail message)
    {
        try
        {
            await using var cmd =
                _db.CreateCommand(
                    "INSERT INTO messages (Title, message, user_id, ticket_id) VALUES ($1, $2, (SELECT id FROM users where email = $3), $4)");
    ...
    public async Task<bool> CustomersTask(string email, string password, int companyId)
    {
        try
        {
            ...
            await using var cmd =
                _db.CreateCommand("INSERT INTO users (email, company_id, role, password) VALUES ($1, $2, $3, $4)");
            cmd.Parameters.AddWithValue(email);
            cmd.Parameters.AddWithValue(companyId);
